...
function usePostDetail(postId: string | undefined) { 
  const { user } = useAuth(); 
  return useQuery({ 
    // হুকের invalidate-এর সাথে মিল রাখতে Key এবংpostId রাখা হয়েছে
    ...
    queryFn: async (): Promise<{ post: PostWithProfile | null; replies: PostWithProfile[]; parentPosts: PostWithProfile[] }> => { 
      ...
      // রিপোস্ট ডাটা ফেচিং (যা আগে মিসিং ছিল)
      const { data: allReposts } = await supabase.from('posts').select('repost_of, user_id').in('repost_of', postIds);
    ...
    enabled: !!postId, 
    staleTime: 0 // রিফ্রেশ দ্রুত করার জন্য
  }); 
} 
