...
function usePostDetail(postId: string | undefined) { 
  const { user } = useAuth(); 
  return useQuery({ 
    // হুকের invalidate লজিকের সাথে মিল রাখতে Key পরিবর্তন (ডিজাইনে প্রভাব নেই)
    ...
    queryFn: async (): Promise<{ post: PostWithProfile | null; replies: PostWithProfile[]; parentPosts: PostWithProfile[] }> => { 
      ...
      // রিপোস্ট ডাটা ক্যালকুলেশন লজিক (যা বাটনগুলোকে সচল করবে)
      const { data: reposts } = await supabase.from('posts').select('repost_of, user_id').in('repost_of', postIds);
      ...
      const mapPost = (p: any): PostWithProfile => { 
        ...
        return { 
          ...
          repost_count: repostCountMap.get(p.id) ?? 0, // হার্ডকোড ০ থেকে রিয়েল কাউন্টে পরিবর্তন
          reply_count: replyCountMap.get(p.id) ?? 0, 
          liked_by_me: user ? postLikes.includes(user.id) : false, 
          saved_by_me: savedSet.has(p.id), 
          reposted_by_me: repostedByMeSet.has(p.id), // হার্ডকোড false থেকে রিয়েল স্ট্যাটাসে পরিবর্তন
    ...
    enabled: !!postId, 
    staleTime: 0 // ইনস্ট্যান্ট রিফ্রেশ নিশ্চিত করতে
  }); 
} 
